...
def sync_rtc_with_ds3231():
    ...
    try:
        ...
        ds3231_plus = machine.Pin(2, machine.Pin.OUT)
        ds3231_plus.value(1)

        # יצירת אובייקט I2C (בהנחה שהשימוש בפינים 21 ו-22)
        # בפין 12 חובה להשתמש דווקא ב softI2C
        ds3231_i2c = machine.SoftI2C(scl=machine.Pin(0), sda=machine.Pin(1))
...
WIDTH = 72
HEIGHT = 40
i2c = I2C(0, scl=Pin(6), sda=Pin(5))  # שנה פינים בהתאם לחומרה שלך
...
def main():
    ...
    oled.fill(0)
    date_string = "{:02d}/{:02d}/{:02d} ({})".format(day, month, int(str(year)[-2:]), get_normal_weekday(rtc_week_day))
    time_string = "{:02d}:{:02d}:{:02d}".format(hour, minute, second)
    
    if current_screen % 10 in [0,3,6,9]:
        oled.text(f'{location["name"]}', 0, 0)
    else:
    ...
    oled.text(f"{"   " if altitude > 0 else "  "}{round(altitude,3):.3f}", 0, 21)
    #oled.text(f"S_rise: {sunrise}", 0, 40)
    oled.text(f" {temporal_time}", 0, 32)
    '''
